...
const { RANK_ORDER, effectiveRank, atLeast } = require('./_rank');
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!atLeast(callerRank, 'administrador')) {
      return { statusCode: 403, body: JSON.stringify({ success: false, error: 'Solo un administrador puede cambiar rangos.' }) };
    }

    const body = JSON.parse(event.body || '{}');
    const targetUsername = (body.username || '').trim();
    const newRank = body.rank;

    if (!targetUsername) return { statusCode: 400, body: JSON.stringify({ success: false, error: 'Falta el nombre de usuario.' }) };
    if (!RANK_ORDER.hasOwnProperty(newRank)) {
      return { statusCode: 400, body: JSON.stringify({ success: false, error: 'Rango inválido.' }) };
    }
