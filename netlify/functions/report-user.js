...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (targetProfileId === me.id) return { statusCode: 400, body: JSON.stringify({ success: false, error: 'No puedes reportarte a ti mismo.' }) };

    await supabaseRequest('user_reports', {
      method: 'POST',
      headers: { 'Prefer': 'return=minimal,resolution=ignore-duplicates' },
      body: JSON.stringify({ reporter_id: me.id, target_id: targetProfileId })
    });
